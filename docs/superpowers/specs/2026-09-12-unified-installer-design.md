...
5. Forwards `SIGINT` and `SIGTERM`.
6. Stops both child processes on shutdown.
7. Exits non-zero when startup fails or either required child exits unexpectedly.
