// Ported from web/lib/provider-credential-store.ts (2026-09-10): the web tier
// no longer touches auth.json directly; the daemon owns credential writes,
// locked with the same proper-lockfile protocol pi's AuthStorage uses.
...
type ProviderCredentialType = "api_key" | "oauth";
...
export function authPathResolved(authPath?: string): string {
...
export function storeProviderCredential(
  ...
  return updateStoredCredentials(authPathResolved(authPath), (credentials) => {
...
export async function removeStoredCredentialIfType(
  ...
  return updateStoredCredentials<CredentialRemovalResult>(authPathResolved(authPath), (credentials) => {
