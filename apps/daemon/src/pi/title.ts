// Session-title generation for the daemon: ported verbatim from the web tier
// (web/lib/session-title.ts) so auto-naming no longer needs an in-process Pi
// session in the web process. The temporary shadow Agent shares the source
// session's provider config; tools are stubbed to throw so a naming run can
...
export function sanitizeTitleMessages(messages: AgentMessage[]): AgentMessage[] {
  ...
  for (let index = 0; index < messages.length; index++) {
    ...
    if (message.role === "assistant") {
      ...
      const content = message.content.filter((block: { type?: string; id?: string }) => {
