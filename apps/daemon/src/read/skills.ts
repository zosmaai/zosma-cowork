...
import { runNpx } from "./lib/npx.ts";
import { getAllowedFileRoots, isExistingFilePathAllowed } from "../git/file-access.ts";
import { getProjectTrustStatus } from "./lib/project-trust.ts";
import {
  buildSkillUpdateArgs,
  checkSkillUpdates,
} from "./lib/skill-updates.ts";
import { loadSkillsWithInstallInfo } from "./lib/skills-service.ts";
...
// Toggle the disable-model-invocation frontmatter key on a SKILL.md file.
// Surgical line edit (no YAML re-serialization) to preserve formatting of the
// other fields. Gates mirror the in-app route: allowed roots plus the agent
// dir and the global ~/.agents/skills root (symlinked global skills resolve
// outside the agent dir).
const TOGGLE_KEY = "disable-model-invocation";
const TOGGLE_KEY_RE = /^disable-model-invocation\s*:/m;
...
export async function toggleSkillModelInvocation(input: {
  filePath: string;
  disableModelInvocation: boolean;
}): Promise<{ success: true }> {
  const { filePath, disableModelInvocation } = input;
  if (!existsSync(filePath)) throw new BackendError("entry_not_found", "file not found");
  ...
  const alreadySet = block !== null && TOGGLE_KEY_RE.test(block);
  ...
  if (disableModelInvocation && !alreadySet) {
    ...
    updated = content.replace(new RegExp(`^${TOGGLE_KEY}\\s*:.*\\r?\\n`, "m"), "");
  }
