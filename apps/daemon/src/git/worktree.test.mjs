...
import { findCurrentWorktreePath, listWorktrees, resolveProject } from "./worktree.ts";
...
test("main and linked worktrees share one canonical project root", async (t) => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "zosma-daemon-worktree-"));
  t.after(() => rm(tempRoot, { recursive: true, force: true }));

  const repo = path.join(tempRoot, "repo");
  const linked = path.join(tempRoot, "linked");
  await execFileAsync("git", ["init", repo]);
  await git(repo, ["config", "user.name", "Zosma Daemon Test"]);
  await git(repo, ["config", "user.email", "zosma-daemon-test@example.invalid"]);
