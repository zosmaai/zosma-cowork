...
import { samePath, toNativePath } from "./paths.ts";
...
export async function resolveProject(cwd: string): Promise<ProjectInfo> {
  ...
  try {
    ...
    // git always emits 4 lines here; `[0..2]` are paths, `ref` the branch.
    // noUncheckedIndexedAccess widens `[i]` to `string | undefined`, so
    // assert the guaranteed entries and keep `ref` nullable.
    const commonDirRaw = rawLines[0]!;
    const gitDirRaw = rawLines[1]!;
    const toplevelRaw = rawLines[2]!;
    const ref = rawLines[3];
