...
// Single-flight + short-TTL dedupe for daemon reads. Concurrent callers for
// the same key share ONE request while in flight; after a caller settles the
// response it is reused for ttlMs so idle remount/re-render storms stop
// re-hitting the daemon. ttlMs 0 = coalesce in-flight only, always refetch
// after (no staleness). Rejections are never cached.
...
function dedupeWithTtl<T>(
	...
	const promise: Promise<T> = run()
		.then((value) => {
			dedupeCache.set(key, { promise, expires: Date.now() + ttlMs });
			return value;
		})
		.catch((error) => {
	...
	dedupeCache.set(key, { promise, expires: 0 }); // in flight: share with concurrent callers
	return promise;
}

export function listSessions(force = false): Promise<SessionsResponse> {
	// Force scans are explicit fresh reads — never dedupe those. Plain reads
	// coalesce only while in flight (ttl 0), so consecutive callers never
	// duplicate work but nothing goes stale.
	if (force) return apiFetch("/api/v1/sessions?force=1", { cache: "no-store" });
	return dedupeWithTtl(`sessions:plain`, 0, () =>
		apiFetch("/api/v1/sessions", { cache: "no-store" }),
	);
}
...
export function getSessionContext(
	...
	return apiFetch(
		`/api/v1/sessions/${encodeURIComponent(sessionId)}/context?${params}`,
	);
}
...
export function getModels(cwd?: string): Promise<ModelsResponse> {
	// Model catalog is expensive when the daemon's provider (llama-swap) is
	// down — it re-probes on every call. Cache 10s: mount/remount + session
	// cwd changes stop re-hitting the daemon, and the catalog only changes on
	// config edits which are far slower than the window.
	const key = cwd ? `models:${cwd}` : "models:";
	return dedupeWithTtl(key, 10_000, () =>
		apiFetch(
			cwd ? `/api/v1/models?cwd=${encodeURIComponent(cwd)}` : "/api/v1/models",
		),
	);
}
...
export function listSkills(cwd?: string): Promise<SkillsResponse> {
	return apiFetch(
		cwd ? `/api/v1/skills?cwd=${encodeURIComponent(cwd)}` : "/api/v1/skills",
	);
}
...
export function listPlugins(cwd?: string): Promise<PluginsResponse> {
	return apiFetch(
		cwd ? `/api/v1/plugins?cwd=${encodeURIComponent(cwd)}` : "/api/v1/plugins",
	);
}
