...
export function proxy(request: NextRequest) {
  ...
  // session cookie; without one neither app HTML nor its API is available.
  // The onboarding routes stay open or nobody could ever sign in, and
  // Basic-auth callers (scripts, remote clients) are already authenticated.
  if ((isApiRequest || isAppRoute) && !basicAuthenticated) {
    ...
    if (needsZosmaSession({ pathname, hasValidSession })) {
