...
test("renders idle rounded composer with preserved controls", () => {
  const html = renderToStaticMarkup(
    React.createElement(
      I18nProvider,
      null,
  ...
  for (const token of ["composer-shell", "composer-card", "composer-input-row", "composer-textarea", "composer-toolbar", "composer-send"]) assert.match(html, new RegExp(token));
  assert.doesNotMatch(html, /composer-stop/);
...
test("renders streaming composer with one stop and queue actions", () => {
  ...
  assert.equal((html.match(/composer-stop/g) ?? []).length, 1);
  assert.doesNotMatch(html, /composer-send/);
  assert.match(html, /Steer/);
  assert.match(html, /Follow-up/);
});

test("keeps composer source handlers and textarea cap contract", async () => {
  ...
  assert.doesNotMatch(source, /maxHeight: 200/);
});
