...
export const LoginScreen = () => {
	...
	// Reset spinner when the deep-link callback fails (token rejected, network
	// error, etc.) so the user can try signing in again.
	useEffect(() => {
		const handle = () => {
			setLoading(false);
	...
	const handleGoogleSignIn = async () => {
		...
		setError(null);
		try {
			// disableRedirect: true — prevents the auth client from calling
			// window.location.href. We open the URL ourselves via openUrl() so it
			// launches in the system browser (Chrome) instead of the WebView.
			...
			// loading stays true until the deep-link callback in useZosmaAuth fires.
		} catch {
