...
export function ActivityBlock({ toolCalls, active = true, maxTrail = 3 }: ActivityBlockProps) {
	...
	return (
		...
		>
			...
			{trail.length > 0 && (
				<div className="mt-1 flex flex-wrap items-center gap-x-1.5 gap-y-0.5 pl-6 text-[0.8125rem] opacity-60">
					{trail.map((a, i) => (
...
export function ActivityRecap({ toolCalls }: { toolCalls: ToolCallInfo[] }) {
	...
	return (
		<div className="my-1 flex items-center gap-1.5 text-[0.8125rem] text-muted-foreground">
			{anyError ? (
