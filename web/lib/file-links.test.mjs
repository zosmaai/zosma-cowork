...
test("preserves encoded path characters while rejecting Windows relative escapes", async () => {
  const { resolveLocalFileHref } = await loadSubject();
  assert.equal(resolveLocalFileHref("query%3F.json", "/home/me/project"), "/home/me/project/query?.json");
  assert.equal(resolveLocalFileHref("nested%23file.txt", "C:/repo"), "C:/repo/nested#file.txt");
  assert.equal(resolveLocalFileHref("..\\outside.txt", "C:/repo"), null);
});
