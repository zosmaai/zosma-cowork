...
export function SessionSidebar({ selectedSessionId, onSelectSession, onNewSession, initialSessionId, skipInitialProjectSelection, onInitialRestoreDone, refreshKey, onSessionDeleted, selectedCwd: selectedCwdProp, onCwdChange, onOpenFile, explorerRefreshKey, onExplorerRefresh, onAtMention, onAtMentions, onBackgroundTaskDone, onRunningSessionIdsChange, onAddFolder, onSelectFolder, validatedProject, onToggleSidebar }: Props) {
  const { t } = useI18n();
  ...
  const projectFor = useCallback((cwd: string | null): ProjectSelection | null => {
    if (!cwd) return null;
    // Shell-side validation resolves identity before a custom path becomes active,
  ...
  const selectedProject = useMemo(() => projectFor(explorerCwd), [explorerCwd, projectFor]);

  // Per-workspace activity counts derive from the unfiltered session set so
  // badges stay accurate while search hides non-matching sessions.
  ...
  // Expansion is intentionally memory-only: search computes effective
  // expansion without writing this set, and nothing persists it.
  const selectedWorkspaceKey = selectedProject?.key ?? null;
  ...
  // Hiding the selected workspace (collapse) must close the worktree
  // dropdown without touching cwd.
  useEffect(() => {
  ...
  const handleWorkspaceSelect = useCallback((row: WorkspaceBrowserRow) => {
    setWorkspaceQuery("");
    if (row.key === selectedProject?.key) {
      setExpandedWorkspaceKeys((keys) => (keys.has(row.key) ? keys : new Set(keys).add(row.key)));
      return;
    }
    setSelectedCwd(workspaceActivationCwd(row, selectedProject?.key ?? null, selectedCwd));
  ...
  return (
    <div className={`workspace-sidebar${selectedSessionId === null ? " is-new-session" : ""}`} style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      {/* Header */}
      <div
      ...
      <div
        ...
        style={{ flex: explorerOpen && explorerCwd ? "1 1 0" : "1 1 auto", minHeight: explorerCwd ? 80 : 0, overflowY: "auto", padding: "0" }}
      >
        {loading && (
        ...
        {!loading && !error && workspaceInputs.length === 0 && workspaceQuery.trim() === "" && (
        ...
        {visibleWorkspaces.map((row) => {
          ...
          const isSelected = row.key === selectedProject?.key;
          ...
          return (
            <div className="workspace-group" key={row.key}>
              <div className="workspace-row" style={{ display: "flex", alignItems: "stretch", borderBottom: "1px solid var(--border)" }}>
                <button type="button" aria-expanded={isExpanded} aria-controls={groupId} onClick={() => toggleWorkspaceExpansion(row.key)}                                    aria-label={isExpanded ? t("sidebar.collapseWorkspace") : t("sidebar.expandWorkspace")}
                ...
                <button
                  type="button"
                  onClick={() => handleWorkspaceSelect(row)}
          ...
          <button
            className="workspace-worktree-guide"
            type="button"
            aria-disabled="true"
            tabIndex={-1}
        ...
        {searching && !hasQueryMatches && (
          <div role="status" style={{ padding: "12px 14px", display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "var(--text-muted)" }}>
            <span style={{ flex: 1 }}>{t("sidebar.noWorkspaceMatches")}</span>
            <button
              type="button"
              onClick={() => setWorkspaceQuery("")}
              style={{
    ...
    >
      {confirmDelete ? (
        ...
        <>
          ...
          <button type="button" className="session-row-main"                        onClick={onClick}
            style={{
              flex: 1,
              minWidth: 0,
              display: "flex",
