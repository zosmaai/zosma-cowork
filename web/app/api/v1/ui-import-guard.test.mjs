...
// it walks lib/ + v1 route source and fails (reporting file:line)
// if any import targets a UI module, so the CI `test` check catches the
// regression instead of silent coupling.
const ROOT = process.cwd();
const SCAN_DIRS = ["app/api/v1"];
const SCAN_FILES = ["lib/api-contracts.ts", "lib/backend-errors.ts"];
...
test("contract + v1 route source never imports a UI module", () => {
  const violations = [];
  const files = SCAN_FILES.map((f) => join(ROOT, f));
  for (const dir of SCAN_DIRS) {
    files.push(...collect(join(ROOT, dir)));
  }
